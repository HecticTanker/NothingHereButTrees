...
texture Scroll
< 
	string UIName = "Scrolling Map";
    //string ResourceType = "Cube"; 
>;

sampler2D ScrlSampler = sampler_state 
{
	Texture = <Scroll>;
	MinFilter = MinFilterBest;
	MagFilter = MagFilterBest;
...
//  I hard coded the EnvBumpScale because it was confusing the artist
	...
	float4 ScrlTexture = tex2D(ScrlSampler, (texCoord0 - Time*TexCoordTransform_0.zw));
	...
	spectex3 *= ScrlTexture.x;
//	color.xyz = lerp(color,baseTexture,spectex3);

	float3 housecolor = {.7,.05,.05};

	color.xyz += spectex3 * housecolor * NODEffectMultiplier;
//	color.xyz += spectex3 * baseTexture;
